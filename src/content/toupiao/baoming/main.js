...
function delRegFormUploadImage(boxid)
{
	if($("#"+boxid+" .imgboxdiv img").is("img"))
	{
		
		_delimgguid = $("#"+boxid+" .imgboxdiv img").attr("id");
		$("#"+boxid+" .imgboxdiv").html("<i class='icon-picture'></i><i class=\"icon-plus\"></i>");
		$("#"+boxid+" .imgdelbtn").hide();
		$("#"+boxid+"").removeClass("selectimgboxlloading");
		imgguidsnew = new Array();
		for(i in imgguids)
		{
			if(imgguids[i] != _delimgguid)
			{
				imgguidsnew.push(imgguids[i]);
			}
		}
		imgguids = imgguidsnew;
		setImageGuidsVal();
		_uploadnum -- ;
		
		param = {};
		param.imgName = _delimgguid;
		param.action = "delUp.do";
	
		$.post(serverroot1+"/toupiao/upFile/delUp.do",param,function(json){
