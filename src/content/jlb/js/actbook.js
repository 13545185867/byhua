	...
	$('#baomingForm2').submit(function() {
		...
		if(submitType == 'ok'){
			...
			if(window.confirm('你确定要报名吗？')){	
				jQuery.ajax({
					url:serverroot+'/jlb/comhousebook.html',
					data:$('#baomingForm2').serialize(),
					type:"POST",
					datatype : "json",
					beforeSend:function(){ 
					...
					success:function(data){
						if (data.success) {
							showTip("报名成功",3000,refresh,'fydDIV');
						}else{
							showTip("报名失败",3000,refresh,'fydDIV');
						}
					}
				});
			}
			...
			if(window.confirm('你确定要取消报名吗？')){	
				jQuery.ajax({
					url:serverroot+'/jlb/comcancelbook.html',
					data:$('#baomingForm2').serialize(),
					type:"POST",
					datatype : "json",
					beforeSend:function(){ 
