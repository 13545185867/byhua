...
public void list(HttpServletResponse response,HttpServletRequest request ) throws Exception{
   ...
   // System.out.println(sb);
	...
	WXPay wxp = new WXPay(wxpc,null,false,true);
	//System.out.println(6);
	Map<String, String> m = null ;
	try{
	 m =  wxp.processResponseXml(sb.toString())	;
	}catch(Exception e){
		e.printStackTrace();
	}
	...
	if(order.getPaystatus().equals(1)){
		...
		//	System.out.println(1);
			...
			userservice.setStarttime(startdate);
		     userservice.setUserid(or.getCreator());
...
@RequestMapping(value="houseNotify",method = {RequestMethod.GET,RequestMethod.POST})
public void houseNotify(HttpServletResponse response,HttpServletRequest request ) throws Exception{
   ...
   // System.out.println(sb);
	...
	//System.out.println(3);
    order.setPayuserid(openid);
    order.setCreateuser(openid);
    xfOrderService.doEdit(order);
    order = xfOrderService.queryById(out_trade_no);
    XfHouse xfhouse = new XfHouse();
    xfhouse.setStatus("3");
    xfhouse.setId(order.getHouseid());
