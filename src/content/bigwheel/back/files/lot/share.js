...
//	var url = '/getJsApiTicket.html?wuid='+wuid;
	var url ='';
	if(!wuid){//null或者undefined
		 url = serverroot + '/bigwheel/getJsApiTicket.html?uid='+activityData.jwid +'&activityid=1';
	}else{
		url = serverroot + '/bigwheel/getJsApiTicket.html?uid='+ activityData.jwid +'&activityid='+wuid;
	}
	$.ajax({
        url: url,
        ...
        success: function (data) {
			 //  data =  eval('(' + data + ')');
        	console.log(data.model.appid+','+data.model.ticket);
        	var appId=data.model.appid;//在jsp页面定义
        	var jsApiTicket=data.model.ticket;//在jsp页面定义
        	var timestamp=new Date().getTime()+'';
        	var nonceStr=timestamp+parseInt(Math.random()*100000)+'';
        	var strSHA1=hex_sha1("jsapi_ticket="+jsApiTicket+"&noncestr="+nonceStr+"&timestamp="+timestamp+"&url="+window.location.href);//SHA1加密
        	wx.config({
        		debug: false,
        	    appId: appId,
        	    timestamp: timestamp,
...
function initShareInfo(shareTitle, shareSummary, shareImgUrl, shareUrl,successCallback,wuid){
