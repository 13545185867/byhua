...
public void tomember(HttpServletResponse response,HttpServletRequest request) throws Exception{
    ...
    if(user !=null && user.getMobile()==null){
		...
		house.setIsdelete("0");
		house.setType("0");
		velocityContext.put("diyalist",houseactivityService.queryList(house));
		
		//转让列表
		house.setType("1");
		velocityContext.put("zhuanranglist",houseactivityService.queryList(house));
...
public void orderDetail(@RequestParam(required = true, value = "id" ) String id,HttpServletResponse response,HttpServletRequest request)throws Exception{
       ...
       // System.out.println("3");
        ...
        System.out.println(responseMap.get("result_code"));
...
@RequestMapping(value="houseorderlist",method = {RequestMethod.GET,RequestMethod.POST})
public void houseorderlist(@ModelAttribute  XfOrder   query,HttpServletResponse response,HttpServletRequest request,
			@RequestParam(required = false, value = "pageNo", defaultValue = "1") int pageNo,
			@RequestParam(required = false, value = "pageSize", defaultValue = "10") int pageSize) throws Exception{
		...
		String viewName = "jlb/houseorder-list.vm";
		ViewVelocity.view(request,response,viewName,velocityContext);
}
/**
 * 列表页面
...
@RequestMapping(value="myFollowed",method = {RequestMethod.GET,RequestMethod.POST})
public void myFollowed(@ModelAttribute XfFollow query,HttpServletResponse response,HttpServletRequest request,
			@RequestParam(required = false, value = "pageNo", defaultValue = "1") int pageNo,
			@RequestParam(required = false, value = "pageSize", defaultValue = "10") int pageSize) throws Exception{
	 	...
	 	VelocityContext velocityContext = new VelocityContext();
	 	String userid = ((Userinfo)request.getSession().getAttribute("OPERATE_ACTIVITY_USER")).getOpenid();
	 	query.setFollowuser(userid);
		pageQuery.setQuery(query);
		velocityContext.put("pageInfos",SystemTools.convertPaginatedList(xfFollowService.queryPageList(pageQuery)));
		String viewName = "jlb/myFollow-list.vm";
		ViewVelocity.view(request,response,viewName,velocityContext);
}
...
public void houseList(@RequestParam(required = true, value = "type" ) String type,HttpServletResponse response,HttpServletRequest request,
		@RequestParam(required = false, value = "pageNo", defaultValue = "1") int pageNo,
		@RequestParam(required = false, value = "pageSize", defaultValue = "10") int pageSize) throws Exception{
 	VelocityContext velocityContext = new VelocityContext();
	String areaid = request.getParameter("areaid");
		PageQuery<Houseactivity> query = new PageQuery<Houseactivity>();
		Houseactivity house = new Houseactivity();
		house.setIsdelete("0");
		if(type != null && !(""==type)){
			house.setType(type);
		...
		if(areaid != null && !(""==areaid)  ){
			house.setAreaid(Integer.parseInt(areaid));
		}
		query.setQuery(house);
		velocityContext.put("pageInfos",SystemTools.convertPaginatedList(houseactivityService.queryPageList(query)));
	 
	velocityContext.put("type",type);
	velocityContext.put("areaid",areaid);
		String viewName = "jlb/house-list.vm";
		ViewVelocity.view(request,response,viewName,velocityContext);
}
...
@RequestMapping(value = "/comhousebook",method ={RequestMethod.GET, RequestMethod.POST})
@ResponseBody
public AjaxJson comhousebook(HttpServletRequest request ) {
	AjaxJson j = new AjaxJson();
	String activityid = request.getParameter("activityid");
	Userinfo user = (Userinfo)request.getSession().getAttribute("OPERATE_ACTIVITY_USER");
	if(user.getMobile() != null && activityid != null  ){
		Houseactivity hact = houseactivityService.queryById(activityid);
		...
		if( hact.getEndtime().getTime()>new Date().getTime() && ( !hact.getStatus().equals("1")  )){
			Activitybookrecord abr = new Activitybookrecord();
			abr.setActivityid(activityid);
			abr.setActivitytitle(hact.getTitle());
			abr.setBooktime(new Date());
			abr.setDeatilurl("jlb/housedetail.html?id="+hact.getId());
			abr.setOpenid(user.getOpenid());
			activitybookrecordService.doAdd(abr);
			j.setSuccess(true);
			j.setMsg("报名成功");
...
@RequestMapping(value = "/comcancelbook",method ={RequestMethod.GET, RequestMethod.POST})
@ResponseBody
public AjaxJson comcancelbook(HttpServletRequest request) {
	AjaxJson j = new AjaxJson();
	String activityid = request.getParameter("activityid");
	Userinfo user = (Userinfo)request.getSession().getAttribute("OPERATE_ACTIVITY_USER");
	if(user.getMobile() != null && activityid != null ){
...
public void areaindex(HttpServletResponse response,HttpServletRequest request) throws Exception{
	...
	//团购
	house.setType("2");
	velocityContext.put("tuangoulist",houseactivityService.queryList(house));
	//在线选房
	//house.setType("3");
	//velocityContext.put("onlinesellist",houseactivityService.queryList(house));
...
@RequestMapping(value="housedetail",method = {RequestMethod.GET,RequestMethod.POST})
public void housedetail(@RequestParam(required = true, value = "id" ) String id,HttpServletResponse response,HttpServletRequest request)throws Exception{
	VelocityContext velocityContext = new VelocityContext();
	String viewName = "jlb/house_detail.vm";
	String openid = ((Userinfo)request.getSession().getAttribute("OPERATE_ACTIVITY_USER")).getOpenid();
	boolean vipuser =     (Boolean) request.getSession().getAttribute("vipuser");
	Houseactivity house = houseactivityService.queryById(id);
	if(house == null ||(!vipuser &&  house.getIsvipview().equals("1"))){
		viewName =  "jlb/noauth.vm";
		ViewVelocity.view(request,response,viewName,velocityContext);
	}
	if(house.getViewcount() == null){
		house.setViewcount(1);
	}else{
		house.setViewcount(house.getViewcount()+1);
	}
	velocityContext.put("info",house);
	if(( null!=house.getStatus() && house.getStatus().equals("1")) || new Date().getTime() > house.getEndtime().getTime()){
		velocityContext.put("expire","1");
	}else{
		velocityContext.put("expire","0");	
	}
